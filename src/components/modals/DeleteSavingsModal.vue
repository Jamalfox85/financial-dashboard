  ...
  >
    ...
    <div class="form-wrapper">
      <form
        ...
        @submit.prevent="deleteSavingsGoal()"
      >
        <p class="mb-5">Are you sure you want to delete this goal?</p>
        <button type="submit" class="submit-bttn">Delete Goal</button>
      </form>
    </div>
  </vue-final-modal>
</template>
...
export default {
  ...
  setup() {
    const { mutate: DELETE_GOAL } = useMutation(gql`
      mutation DELETE_GOAL($id: uuid!) {
        delete_savings(where: { id: { _eq: $id } }) {
          returning {
            id
          }
        }
  ...
  methods: {
    deleteSavingsGoal() {
      this.DELETE_GOAL({ id: this.goal.id });
      this.closeModal();
    },
    closeModal() {
      this.$emit("close");
